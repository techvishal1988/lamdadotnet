...
```bash
dotlambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
dotlambda$ sam local start-api
dotlambda$ curl http://localhost:3000/
```
...
```bash
dotlambda$ sam logs -n HelloWorldFunction --stack-name dotlambda --tail
```
...
```bash
dotlambda$ dotnet test test/HelloWorld.Test
```
...
```bash
sam delete --stack-name dotlambda
```
